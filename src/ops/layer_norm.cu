...
void LayerNorm::forward_kernel_wrapper(LayerNormMeta const *m,
  ...
  if (m->input_type[0] == DT_FLOAT) {
    ...
    LayerNorm::forward_kernel<__ff_bfloat16>(
        m,
...
void LayerNorm::inference_kernel_wrapper(LayerNormMeta *m,
  ...
  if (bc->num_finetuning_fwd_requests() > 0) {
    ...
    if (m->input_type[0] == DT_FLOAT) {
      ...
      checkCUDA(cudaMemcpyAsync(
          m->input_activation,
          input.get_bfloat16_ptr() + first_token_offset * in_dim,
          data_type_size(m->input_type[0]) * num_peft_tokens * in_dim,
          cudaMemcpyDeviceToDevice,
          stream));
    } else {
...
void LayerNorm::backward_kernel_wrapper(
  ...
  if (m->output_type[0] == DT_FLOAT) {
    ...
    LayerNorm::backward_kernel(m,
                               ...
                               input_grad.get_bfloat16_ptr(),
                               gamma.get_bfloat16_ptr(),
                               gamma_grad.get_bfloat16_ptr(),
                               beta_grad.get_bfloat16_ptr(),
                               stream);
  } else {
    assert(false && "Unsupported data type");
  }
