...
void ResidualLayerNorm::inference_kernel_wrapper(
  ...
  if (m->input_type[0] == DT_FLOAT) {
    ...
    ResidualLayerNorm::inference_kernel<__ff_bfloat16>(
        m,
...
void ResidualLayerNorm::backward_kernel_wrapper(
  ...
  if (m->output_type[0] == DT_FLOAT) {
    ...
    backward_kernel(
        ...
        residual1_grad.get_bfloat16_ptr(),
        m->use_two_residuals ? residual2_grad.get_bfloat16_ptr() : nullptr,
        m->elementwise_affine ? gamma.get_bfloat16_ptr() : nullptr,
...
void ResidualLayerNorm::peft_bwd_kernel_wrapper(
  ...
  if (m->output_type[0] == DT_FLOAT) {
    ...
    peft_bwd_kernel(m,
                    ...
                    input_grad.get_bfloat16_ptr(),
                    residual1_grad.get_bfloat16_ptr(),
                    m->use_two_residuals ? residual2_grad.get_bfloat16_ptr()
                                         : nullptr,
                    m->elementwise_affine ? gamma.get_bfloat16_ptr() : nullptr,
                    stream);
  } else {
