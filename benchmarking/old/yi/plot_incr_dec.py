...
def plot_tpot_vs_throughput(results_dict):
    ...
    # Adjust layout
    plt.tight_layout()
    plt.savefig(f'./plots/incr_decoding/yi/throughput_vs_latency.pdf', bbox_inches='tight')
    plt.show()
...
def plot_ttft_bars(results_dict):
    ...
    # Adjust layout
    plt.tight_layout()
    plt.savefig(f'./plots/incr_decoding/yi/ttft.pdf', bbox_inches='tight')
    plt.show()
...
def plot_queue_time_bars(results_dict):
    ...
    # Adjust layout
    plt.tight_layout()
    plt.savefig(f'./plots/incr_decoding/yi/queueing_time.pdf', bbox_inches='tight')
    plt.show()
...
def get_dfs(models, datasets, max_tokens_per_batch, arrival_rates):
    incr_dec_req_profiling_fps = {
        IncrDecExpKey(model, dataset, max_tokens, arrival_rate): 
            f"./data/incr_decoding/yi/inference_request_profiling_{dataset}_{model}_tensor_parallelism_2_max_requests_per_batch_8_max_tokens_per_batch_{max_tokens}_arrival_rate_{arrival_rate}_num_warmup_requests_10.csv"
        for model in models
    ...
    incr_dec_step_profiling_fps = {
        IncrDecExpKey(model, dataset, max_tokens, arrival_rate): 
            f"./data/incr_decoding/yi/step_profiling_{dataset}_{model}_tensor_parallelism_2_max_requests_per_batch_8_max_tokens_per_batch_{max_tokens}_arrival_rate_{arrival_rate}_num_warmup_requests_10.csv"
        for model in models
        for dataset in datasets
        for max_tokens in max_tokens_per_batch
        for arrival_rate in arrival_rates
...
if __name__ == "__main__":
    models=["01-ai/Yi-1.5-34B".replace("/", "_").lower()]
    ...
    target_df = incr_dec_req_profiling_dfs[IncrDecExpKey(model="01-ai/Yi-1.5-34B".replace("/", "_").lower(), dataset='sharegpt', max_tokens_per_batch=128, arrival_rate='0.000000')]
    ...
    if not os.path.exists('./plots/incr_decoding/yi'):
        os.makedirs('./plots/incr_decoding/yi')
